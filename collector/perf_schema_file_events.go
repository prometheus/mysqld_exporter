...
func (ScrapePerfFileEvents) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, constLabels prometheus.Labels, logger log.Logger) error {
	...
	for perfSchemaFileEventsRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_total", "The total file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
			prometheus.CounterValue, float64(countRead),
			eventName, "read",
		)
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_seconds_total", "The total seconds of file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
			prometheus.CounterValue, float64(timeRead)/picoSeconds,
			eventName, "read",
		)
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_bytes_total", "The total bytes of file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
			prometheus.CounterValue, float64(bytesRead),
			eventName, "read",
		)
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_total", "The total file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
		...
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_seconds_total", "The total seconds of file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
			prometheus.CounterValue, float64(timeWrite)/picoSeconds,
			eventName, "write",
		)
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_bytes_total", "The total bytes of file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
			prometheus.CounterValue, float64(bytesWrite),
			eventName, "write",
		)
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_total", "The total file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
			prometheus.CounterValue, float64(countMisc),
			eventName, "misc",
		)
		ch <- prometheus.MustNewConstMetric(
			newDescLabels(performanceSchema, "file_events_seconds_total", "The total seconds of file events by event name/mode.", constLabels, []string{"event_name", "mode"}),
			prometheus.CounterValue, float64(timeMisc)/picoSeconds,
			eventName, "misc",
		)
	}
	return nil

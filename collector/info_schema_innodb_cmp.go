...
const innodbCmpQuery = `SELECT * FROM information_schema.INNODB_CMP`
	...
	for informationSchemaInnodbCmpRows.Next() {
		...
		for idx, columnName := range columnNames[1:] {
			if metricType, ok := informationSchemaInnodbCmpTypes[columnName]; ok {
				ch <- prometheus.MustNewConstMetric(metricType.desc, metricType.vtype, float64(clientCmpData[idx]), page_size)
			} else {
				...
				ch <- prometheus.MustNewConstMetric(desc, prometheus.UntypedValue, float64(clientCmpData[idx]), page_size)
			}
		}
	}
	return nil
